...
There are three types of testing: unit testing, integration testing and end-to-end 
testing.
...
- Test suites: Linux Test Project(collection of tools), static code analyzers(Coverty, coccinelle, smatch, sparse), module tests(KUnit), fuzzing tools(Trinity, Syzcaller) and subsystem tests.

- Automatic testing: kisskb, 0Day, kernelci, Kerneltests.

In the figure below, we can see that as more and better tools were developed we saw an increase in reported vulnerabilities, with a peak in 2017, after which a steady decrease which may be caused by the amount of tools used to verify patches before being upstreamed.
...
Easy?
This is not always the case, for example this [sample](https://github.com/google/googletest/blob/master/googletest/samples/sample9_unittest.cc) shows a more advanced and imbricated test

## 03 Unikraft's testing framework
Unirkaft's testing framework, `uktest`, has been inspired by KUnit and provides a flexible testing API.
### API Overview
...
### How the tests are run by the kernel
...
### 01. Tutorial: Testing a simple factorial application
...
For this task, we will have to modify the existing factorial application by adding a new function that computes if a number is prime.
We will add a new testsuite for this function.
...
### 04. Add a test suite with four test for nolibc
We want to add a new test suite for nolibc with four test cases in it.
...
### 05. Tutorial. Using KLEE for symbolic execution (extra)
...
object 0: text: ^.*.*.*
```
This is just a quick example of the power of symbolic execution, but it comes with one great problem: patch explosion.
