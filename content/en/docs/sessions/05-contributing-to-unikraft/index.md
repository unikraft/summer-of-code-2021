...
Go to the `work/01-tut-porting/libs/libhogweed/` directory and follow the bookmars marked with `USOC_X`, where `X` is the index of the item in the list, from the files specified in the sections below.
...
You can also check the library's `README.md` for additional informations.
...
Move the `work/02-task-porting/src/apps/app-kdtree` application in the `$UK_APPS` directory, complete its `Makefile`, and use it to build an unikernel with our ported library as dependency!
